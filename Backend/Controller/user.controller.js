...
const loginUser = async (req, res) => {
    ...
    try {
        const isuserExist= await userModel.findOne({email:req.body.email});
        if(!isuserExist) {
            return res.status(400).json({ message: "User does not exist. Register first to login" });
        }
        const result =await bcrypt.compare(req.body.password, isuserExist.password);
        if(!result) {
            return res.status(400).json({ message: "Invalid credentials" });
        }
        const token = jwt.sign({userData:isuserExist},"rishi",{expiresIn:"1d",});
        if(!token){
